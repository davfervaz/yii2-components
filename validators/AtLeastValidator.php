...
/**
 * Original author is Sidney Lins.
 * Forked from https://github.com/code-on-yii/yii2-at-least-validator
 */

namespace dlds\components\validators;
...
class AtLeastValidator extends Validator
{
    ...
    /**
     * @var integer the maximum required quantity of attributes that may be filled.
     * Defaults to false.
     */
    public $max = false;
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            if ($this->max) {
                $this->message = 'You must fill at least {min} and not more than {max} of the attributes {attributes}.';
            } else {
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        if (!$chosen || $chosen < $this->min || $chosen > $this->max) {
